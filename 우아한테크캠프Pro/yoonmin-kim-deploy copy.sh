...
nginx_start() {
        cd $APPLICATION_PATH/docker-nginx

        sudo sed -i 's/serveripbinding/'$IP'/g' nginx.conf

        sudo docker build -t nextstep/reverse-proxy:0.0.2 .

        sudo docker run -d -p 80:80 -p 8080:8080 -p 443:443 --name proxy nextstep/reverse-proxy:0.0.2
}
